...
def generate_message_text(data: APIResponse) -> str:
    ...
    messages.append("Аудит сообщества завершен.\nОбщий результатРезультат Сообщество набрало {}% по итогам проверки всех ключевых параметров, "
                    "которые влияют на привлечение клиентов и подписчиков. "
                    "Ниже представлены результаты анализа по основным критериям:\n".format(ceil(data.score)))
